...
- **See `README.md`** for full Supabase, local, and Vercel deployment instructions
...
Express 5 API server with cookie-parser. Routes at `/api`: auth, profiles, income, expenses, obligations, savings, scores, loans, advisory, reports, dashboard, timeline, ask-ai, financial-products. Snapshot service auto-updates current month's financial summary on every CRUD operation (atomic upsert). AI chat uses OpenAI integration (lazy-initialized, SSE streaming). Financial products CRUD has admin-only write middleware (`requireAdmin`). Users table has `is_admin` boolean column. Seed script auto-populates financial products on first startup (39 products — FDs are split per tenure: 1yr/2yr/3yr/5yr rows per bank). Financial products table has both legacy text fields (`interestRate`, `minimumBalance`, `tenure`, `fees`) and structured numeric fields (`interestRateMin/Max`, `minimumBalanceValue`, `tenureMonthsMin/Max`, `feeValue`) plus classification fields (`productSubcategory`, `targetSegment`, `currency`, `isActive`). PUT route only updates new fields when explicitly provided (backward-compatible with legacy clients). Recommendation Matching Engine (`recommendationEngine.ts`) matches user financial condition to bank products from DB — returns up to 3 products and optional caution message. Included in both dashboard and advisory API responses as `bestNextOptions`.
